...
#Run randomcv wrfvar to perturb ICs
  ...
  for NE in `seq 1 $NUM_ENS`; do 
    ...
    if [[ `tail -n1 $id/rsl.error.0000 |grep successful` ]]; then continue; fi
  ...
  for NE in `seq 1 $NUM_ENS`; do
    ...
    cp $id/wrfvar_output $WORK_DIR/rc/$DATE/wrfinput_d01_$id
...
if [ $MAX_DOM -gt 1 ]; then
  ...
  for n in `seq 2 $MAX_DOM`; do
    ...
    for NE in `seq 1 $NUM_ENS`; do
      ...
      cp $id/$dm/wrfinput_d02 $WORK_DIR/fc/$DATE/wrfinput_${dm}_$id
