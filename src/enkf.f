...
do iob=1,obs%num
   ...
   if ( use_simulated ) then
      filename="fort.80010"
      if ( obstype(1:7) == 'RadarRV' ) then
         call xb_to_rv(filename,proj,tmp,ix,jx,kx,nv,iob,xlong,znw,xb,0)
      else if ( obstype(1:8) == 'RadarRef' ) then
         call xb_to_rf(filename,proj,tmp,ix,jx,kx,nv,iob,xlong,znw,xb,0)
      else if ( obstype(1:10) == 'RadarHydro' ) then
         call xb_to_hydro(filename,proj,tmp,ix,jx,kx,nv,iob,xlong,znw,xb,0)
...
! prepare x profiles near obs position (xob) for xb calculation
   ...
   do n = 1, nm
     ...
     if (ie<=numbers_en+1) then
       ...
       if ( obstype(1:7) == 'RadarRV' ) then
         call xb_to_rv(filename,proj,xob(:,:,:,:,n,sid+1),ix,jx,kx,nv,iob,xlong,znw,yasend(iob,ie),1)
       else if ( obstype(1:8) == 'RadarRef' ) then
         call xb_to_rf(filename,proj,xob(:,:,:,:,n,sid+1),ix,jx,kx,nv,iob,xlong,znw,yasend(iob,ie),1)
       else if ( obstype(1:10) == 'RadarHydro' ) then
         call xb_to_hydro(filename,proj,xob(:,:,:,:,n,sid+1),ix,jx,kx,nv,iob,xlong,znw,yasend(iob,ie),1)
       else if ( obstype == 'longtitude' .or. obstype == 'latitude  ' .or. obstype == 'min_slp   ' ) then
         call hurricane_center_assimilation(filename,ix,jx,kx,int(obs%position(iob,1)),int(obs%position(iob,2)), &
                                            center_xb,znu,znw,xlong,xlat,proj)
...
!! Specify adaptive observational error (Zhang et al. 2016) for radiance and zero-hydrometeor (1/10 background error)
   var = 0.
   do ie = 1, numbers_en
   ...
   if ( obstype=='Radiance  ' ) then
      error = max( obs%err(iob), sqrt(max((y_hxm*y_hxm-var*fac),0.)) )
      obs%err(iob) = error
   elseif ( obstype=='RadarHydro') then
      error = sqrt(var*fac)*0.1
      obs%err(iob) = error
   else
      error = obs%err(iob)
   endif
   if( obstype=='RadarHydro' .and. obs%dat(iob) == 0. .and. maxval(ya(iob,:)) <= 0.0001 )then
      kick_flag(iob)=1
      cycle obs_assimilate_cycle
   else
      if (obstype=='RadarHydro') then
         if ( my_proc_id==0 ) write(*,'(a,i6,a,f12.4,a,f12.4,a,f9.4,a,f9.4,a,i4,a,i4)') &
            'No.',iob,' '//obstype//' =',obs%dat(iob)*1000, ' ya=', ya(iob,numbers_en+1)*1000, ' y-ya=', y_hxm*1000, &
            ' err=',error*1000,' hroi=',obs%roi(iob,1),' vroi=',obs%roi(iob,2)
      else
         if ( my_proc_id==0 ) write(*,'(a,i6,a,f12.4,a,f12.4,a,f9.4,a,f9.4,a,i4,a,i4)') &
            'No.',iob,' '//obstype//' =',obs%dat(iob), ' ya=', ya(iob,numbers_en+1), ' y-ya=', y_hxm, &
            ' err=',error,' hroi=',obs%roi(iob,1),' vroi=',obs%roi(iob,2)
      endif
      if ( abs(y_hxm)>(error*5.) .and. .not.(obstype=='min_slp   ' .or. obstype=='longtitude' .or. obstype=='latitude  ' &
         .or. obstype=='RadarRef  ' .or. obstype=='RadarHydro' .or. obstype=='Radiance  ') ) then
...
do iob=1,obs%num
  ...
  if ( my_proc_id == 0 ) then
    if ( obs%type(iob) == 'RadarHydro' ) then
      write(ounit,'(a,6f11.2,2i4,6f11.2)') obs%type(iob), &   !type of observation
        ...
        obs%dat(iob)*1000, &              !observation (y^o)
        obs%err(iob)*1000, &              !observation error variance (R)
        yf(iob,numbers_en+1)*1000, &      !observation prior mean (H\bar{x}^f)
        ya(iob,numbers_en+1)*1000, &      !observation posterior mean (H\bar{x}^a)
        sqrt(var_b)*1000, &               !observation prior spread (sqrt{H P^f H^T})
        sqrt(var_a)*1000
    else
      write(ounit,'(a,6f11.2,2i4,6f11.2)') obs%type(iob), &   !type of observation
        center_xb(1), center_xb(2), &     !latitude, longitude
