!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  program enkf_gfs_hybrid_inflation
...
!  call get_var_ij ( 'fort.70010', 'T_2       ', ix, jx, kx )
   call get_ij ( 'fort.70010' , ix, jx, kx )
  call get_times ( 'fort.70010', 'T_2       ', times )
...
!.... get enkf mean
      write(wrf_file,'(a5,i5.5)')'fort.',i_unit
      if ( kk > 1 ) then
         call get_variable3d( wrf_file, var, ii, jj, kk, 1, xm )
      else if ( kk == 1 ) then
         call get_variable2d( wrf_file, var, ii, jj, 1, xm )
      endif

!.... mixing and output to ensemble
      do n = 1, numbers_en

         write(wrf_file,'(a5,i5.5)')'fort.',gfs_unit+n
         if ( kk > 1 ) then
            call get_variable3d( wrf_file, var, ii, jj, kk, 1, gfs )
         else if ( kk == 1 ) then
            call get_variable2d( wrf_file, var, ii, jj, 1, gfs )
         endif 
         ...
         do k = 1, kk
            ...
            do i = rep_is, rep_ie
               ...
               !work(i,j,k) = alpha*xm(i,j,k) + (1.0-alpha)*gfsm(i,j,k) + 0.2*(xa(i,j,k)-xm(i,j,k)) + 0.8*(gfs(i,j,k)-gfsm(i,j,k))
               work(i,j,k) = alpha*xa(i,j,k) + (1.0-alpha)*gfs(i,j,k) 
            end do
            end do
         end do
...
end program enkf_gfs_hybrid_inflation
...
subroutine wrf_var_dimension ( var, ix, jx, kx, ii, jj, kk )
   ...
   if      ( var == 'U_2       ' ) then
      ii = ix + 1
   else if ( var == 'V_2       ' ) then
      jj = jx + 1
   else if ( var == 'W_2       ' .or. var == 'PH_2      ' .or. var == 'PHB       ' ) then
      kk = kx + 1
   else if ( var == 'MU_2      ' .or. var == 'MUB       ' .or. var == 'Q2        '  &
