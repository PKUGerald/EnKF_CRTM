...
!*******************************************************************************
  ...
  p = p + pb 
  call get_variable3d( wrf_file, 'PH_2      ', ix, jx, kx+1, 1, ph )
...
!. calculate WSR-88D radar reflectivity records
...
!....... Get Reflectivity
   if ( use_radar_rf ) then
      call sort_radarRV_data( wrf_file, ix, jx, kx, proj, 'RadarRef  ', &
              datathin_radar  , hroi_radar  , vroi_radar  , grid_id )
   endif
...
!...... get the data number of every radar
     ...
     do_get_raw_data_loop_read : do
        read(10, '(5f12.3)', iostat = iost ) az, el, ra, rw, ref
        if( iost .ne. 0 ) exit
        num = num + 1
...
call get_variable3d( truthfile, 'U_2       ', ix+1, jx, kx, 1, u )
call get_variable3d( truthfile, 'V_2       ', ix, jx+1, kx, 1, v )
call get_variable3d( truthfile, 'T_2       ', ix, jx, kx, 1, t )
call get_variable3d( truthfile, 'QVAPOR    ', ix, jx, kx, 1, qv )
call get_variable3d( truthfile, 'PH_2      ', ix, jx, kx+1, 1, ph )
...
!......... If use_theta = .true. and pressure exist, use theta ('T')
      if ( use_theta .and. data(n,1,1) .gt. 80000. .and. data(n,1,1) .lt. 120000. ) then
        obs%num                 = obs%num + 1
...
!......... PSFC:
   if ( data(n,1,1).ge.50000. .and. data(n,1,1) .le. 120000. .and. abs(data(n,2,1)).lt. 5. ) then
        obs%num                 = obs%num + 1
        obs%dat     (obs%num  ) = data(n,1,1)
        obs%type    (obs%num  ) = 'S'//instrument//'P'
        obs%err     (obs%num  ) = data(n,3,1)
        obs%position(obs%num,1) = x
        obs%position(obs%num,2) = y
        obs%position(obs%num,3) = 1.
        obs%sta     (obs%num,1) = obs_elv(n)
...
!...... If pressure exists, convert Qv to Td
      if ( use_td .and. data(n,1,1) .gt. 80000. .and. data(n,1,1) .lt. 120000. ) then
        obs%num                 = obs%num + 1
...
!. get data
sta = 0
if ( instrument == 'RadarRV   ' .or. instrument == 'RadarRef  ' )then
   do nr = 1, raw%radar_stn_num
...
if ( instrument == 'RadarRV   ' )then
   do nr = 1, raw%radar_stn_num
   do n  = 1, raw%radar(nr)%radar_stn%numObs
       if ( raw%radar(nr)%rv(n) > -1000 )then
          ista = ista + 1
   ...
   enddo
   enddo
else if ( instrument == 'RadarRef  ' )then
   do nr = 1, raw%radar_stn_num
   do n  = 1, raw%radar(nr)%radar_stn%numObs
       if ( raw%radar(nr)%rf(n) > -35 )then
          ista = ista + 1
          data(ista,1) = raw%radar(nr)%rf(n)
          data(ista,2) = raw%radar(nr)%radar_stn%err_rf
          data(ista,3) = raw%radar(nr)%radar_point(n)%ii
          data(ista,4) = raw%radar(nr)%radar_point(n)%jj
          data(ista,5) = raw%radar(nr)%radar_point(n)%hgt
          data(ista,6) = raw%radar(nr)%radar_stn%i_radar
...
do_reports : do n = start_data, ista, inter_data
  ...
  if ( instrument == 'RadarRV   ' )then
     ...
     if ( data(n,1) == 0. )then
        obs%type(obs%num)       = 'RadarHydro'
     else
        obs%type(obs%num)       = 'RadarRef  '
     end if
  end if
  if ( obs%type(obs%num) == 'RadarHydro' )then
     obs%err(obs%num)        = 0.001
  else
  ...
  if ( instrument == 'RadarRef  ' .and. obs%dat(obs%num) <= 0. )then
     obs%roi(obs%num,1)      = hroi*ngxn*2
  else
     obs%roi(obs%num,1)      = hroi*ngxn
  end if
  obs%roi(obs%num,2)      = vroi
