...
!=======================================================================================
  ...
  real, dimension(ix, jx      )       :: xlong, psfc
  real, dimension(ix, jx, kx+1)       :: ph
  real, dimension(ix, jx, kx+1)       :: phb
  real, dimension(ix, jx, kx  )       :: qr, pb
  real                                :: vtm
  real                                :: rv
...
!    get model output u, v, w
     call get_variable3d ( wrf_file, 'U_2       ', ix+1, jx  , kx  , 1, u )     
     call get_variable3d ( wrf_file, 'V_2       ', ix, jx+1  , kx  , 1, v )     
     call get_variable3d ( wrf_file, 'W_2       ', ix, jx  , kx+1  , 1, w )     
     ...
     call get_variable2d ( wrf_file, 'PSFC      ', ix, jx, 1, psfc )
     call terminal_velocity ( ix, jx, kx, ii, jj, kk, qr, psfc, pb, vtm)
     ...
     do l = 1, raw%radar(n)%radar_stn%numObs
        ...
        call calculate_rv ( ix, jx, kx, u, v, w, vtm, xlong, proj, ii, jj, kk,      &
...
!=======================================================================================
  subroutine calculate_rv ( ix, jx, kx, ugrid, vgrid, wgrid, vtm, xlong, proj, ii, jj, kk,  &
                            hh, i_radar, j_radar, elv, rv)
...
!=======================================================================================
  subroutine terminal_velocity ( ix, jx, kx, obs_i, obs_j, obs_k, qr, psfc, pb, vtm)
! Following Sun and Crook (1998)

  implicit none

  integer, intent(in)                      :: ix, jx, kx
  real, intent(in)                         :: obs_i, obs_j, obs_k
  real, dimension(ix, jx, kx), intent(in)  :: qr, pb
  real, dimension(ix, jx    ), intent(in)  :: psfc
  real,                        intent(out) :: vtm
  real, dimension(2,  2,  2 )              :: vtmg
...
!------------------------------------------------------------------------------
  ...
  do i = ii,ii+1
    do j = jj, jj+1
      do k = kk, kk+1
        vtmg(i-ii+1,j-jj+1,k-kk+1) = 5.4 * ( psfc(i,j) / pb(i,j,k) ) ** 0.4 * ( qr(i,j,k) * 1000 ) ** 0.125
      enddo
    enddo
  enddo

  vtm = (( vtmg(1,1,1)*(1-ire) + vtmg(2,1,1)*ire )*(1-jre) + &
         ( vtmg(1,2,1)*(1-ire) + vtmg(2,2,1)*ire )*jre)*(1-kre) + &
        (( vtmg(1,1,1)*(1-ire) + vtmg(2,1,1)*ire )*(1-jre) + &
         ( vtmg(1,2,1)*(1-ire) + vtmg(2,2,1)*ire )*jre)*kre
