...
do m = 1, nv
   ...
   if ( enkfvar(m) == 'MU_2      ' ) i_mu=m
   if ( enkfvar(m) == 'MUB       ' ) i_mub=m
   if ( enkfvar(m) == 'PH_2      ' ) i_ph=m
...
if ( i_mu == 0 ) call get_variable2d(inputfile, 'MU_2      ', ix, jx, 1,    mu  )
...
!..    get data from background
       ...
       if ( i_ph == 0 ) call get_variable3d(inputfile, 'PH_2      ', ix, jx, kx+1, 1, ph )
       if ( i_phb== 0 ) call get_variable3d(inputfile, 'PHB       ', ix, jx, kx+1, 1, phb)
       if ( i_pt == 0 ) call get_variable3d(inputfile, 'T_2       ', ix, jx, kx,   1, pt )
       if ( i_qv == 0 ) call get_variable3d(inputfile, 'QVAPOR    ', ix, jx, kx,   1, qv )
       if ( i_qc == 0 ) call get_variable3d(inputfile, 'QCLOUD    ', ix, jx, kx,   1, qc )
       if ( i_qr == 0 ) call get_variable3d(inputfile, 'QRAIN     ', ix, jx, kx,   1, qr )
       if ( i_mu == 0 ) call get_variable2d(inputfile, 'MU_2      ', ix, jx, 1,    mu )
...
!.. U, V
    if ( obstype(10:10) == 'U' .or. obstype(10:10) == 'V' .or. obstype(10:10) == 'S' ) then
       ...
       i_v = 0
       do m = 1, nv
          if ( enkfvar(m) == 'U_2       ' ) i_u=m
          if ( enkfvar(m) == 'V_2       ' ) i_v=m
       enddo
...
!=======================================================================================
   ...
   if (obstype .eq. 'idealU    ' ) then
      do m = 1, nv
         if ( enkfvar(m) == 'U_2       ' ) then
            xb = xa(i1,j1,k1,m)
         endif
      enddo
   else if (obstype .eq. 'idealV    ' ) then
      do m = 1, nv
         if (enkfvar(m) == 'V_2       ' ) then
            xb = xa(i1,j1,k1,m)
         endif
      enddo 
   else if (obstype .eq. 'idealPT   ' ) then
      do m = 1, nv
         if (enkfvar(m) == 'T_2       ' ) then
            xb = xa(i1,j1,k1,m) 
         endif
      enddo
      ...
      xb = 0.
      do m = 1, nv
         if ( enkfvar(m) == 'PH_2      ' .or. enkfvar(m) == 'PHB       ' ) then
            xb = xb + xa(i1,j1,k1,m)
         endif
      enddo
   endif
...
!=======================================================================================
  ...
  real, dimension(ix, jx, kx  )        :: t, qv, p, pb, qr
  real, dimension(ix, jx      )        :: mu, mub, psfc, terrain
  real                                 :: vtm
    
  integer :: m, ii, jj, kk, i, j, k, i1, j1
  integer :: i_u, i_v, i_w, i_t, i_qv, i_mu, i_ph, i_phb, i_mub, i_pb, i_qr, i_psfc
  ...
  if ( i_u == 0 ) call get_variable3d(inputfile, 'U_2       ', ix+1, jx, kx, 1, u )
  if ( i_v == 0 ) call get_variable3d(inputfile, 'V_2       ', ix, jx+1, kx, 1, v )
  if ( i_w == 0 ) call get_variable3d(inputfile, 'W_2       ', ix, jx, kx+1, 1, w )
  
  if ( kkflag == 0 ) then
     do m = 1, nv
        if ( enkfvar(m) == 'T_2       ' ) i_t=m
        if ( enkfvar(m) == 'QVAPOR    ' ) i_qv=m
        if ( enkfvar(m) == 'PH_2      ' ) i_ph=m
        if ( enkfvar(m) == 'PHB       ' ) i_phb=m
     ...
     if( i_ph <1 ) call get_variable3d(inputfile, 'PH_2      ', ix, jx, kx+1, 1, ph )
     if( i_phb <1) call get_variable3d(inputfile, 'PHB       ', ix, jx, kx+1, 1, phb )
     if( i_qr <1 ) call get_variable3d(inputfile, 'QRAIN     ', ix, jx, kx, 1, qr )
     if( i_mu <1 ) call get_variable2d(inputfile, 'MU_2      ', ix, jx, 1, mu )
     if( i_mub <1) call get_variable2d(inputfile, 'MUB       ', ix, jx, 1, mub )
     if( i_psfc <1 ) call get_variable2d(inputfile, 'PSFC      ', ix, jx, 1, psfc )
     do j = j1, j1+1
     do i = i1, i1+1
...
!  call terminal_velocity ( ix, jx, kx, obs_ii, obs_jj, obs_kk, qr, psfc, pb, vtm)
  vtm=0
  call calculate_rv ( ix, jx, kx, u, v, w, vtm, xlong, proj, obs_ii, obs_jj, obs_kk, obs_hh, radar_ii, radar_jj, radar_elv, xb )
...
!=======================================================================================
  subroutine xb_to_rf(inputfile,proj,xa,ix,jx,kx,nv,iob,xlong,znw,xb,kkflag)
  use constants
  use namelist_define
  use mapinfo_define
  use obs_define
  ...
  real, dimension(ix, jx, kx+1)        :: ph, phb
  real, dimension(ix, jx, kx  )        :: t, qv, p, pb, rf
  real, dimension(ix, jx      )        :: mu, mub, terrain
!  real, dimension(kx          )        :: rft

  integer :: m, ii, jj, kk, i, j, k, i1, j1, k1
  integer :: i_t, i_qv, i_mu, i_ph, i_phb, i_mub, i_pb, i_rf
  real    :: dx, dxm, dy, dym, dz, dzm
  ...
  i_t  = 0 ; i_qv =  0 ; i_mu = 0; i_mub = 0; i_rf = 0;
  i_ph = 0 ; i_phb = 0 ; i_pb = 0 ;

  do m=1,nv
     if ( enkfvar(m) == 'REFL_10CM ' ) i_rf=m
  enddo
  if( i_rf > 0 ) rf(i1:i1+1,j1:j1+1,1:kx)=xa(1:2,1:2,1:kx,i_rf)
  if( i_rf < 1 ) call get_variable3d(inputfile, 'REFL_10CM ', ix, jx, kx, 1, rf )
...
!  rft(1:kx) = dym*(dx*rf(i1+1,j1,1:kx) + dxm*rf(i1,j1,1:kx)) + dy*(dx*rf(i1+1,j1+1,1:kx) + dxm*rf(i1,j1+1,1:kx))
  if (obs_kk .le. 1) then
     xb = dym*(dx*rf(i1+1,j1,1) + dxm*rf(i1,j1,1)) + dy*(dx*rf(i1+1,j1+1,1) + dxm*rf(i1,j1+1,1))
  else
!     xb = ( dzm*rft(k1) + dz*rft(k1+1) )
     xb = ( dzm * (dym*(dx*rf(i1+1,j1,k1) + dxm*rf(i1,j1,k1)) + dy*(dx*rf(i1+1,j1+1,k1) + dxm*rf(i1,j1+1,k1))) + &
            dz * (dym*(dx*rf(i1+1,j1,k1+1) + dxm*rf(i1,j1,k1+1)) + dy*(dx*rf(i1+1,j1+1,k1+1) + dxm*rf(i1,j1+1,k1+1))) )
  endif

  end subroutine xb_to_rf

!=======================================================================================
  subroutine xb_to_hydro(inputfile,proj,xa,ix,jx,kx,nv,iob,xlong,znw,xb,kkflag)
  use constants
  use namelist_define
  use mapinfo_define
  use obs_define
  ...
  real, dimension(ix, jx, kx+1)        :: ph, phb
  real, dimension(ix, jx, kx  )        :: t, qv, p, pb, qr, qs, qg
  real, dimension(ix, jx      )        :: mu, mub, terrain
  real, dimension(2,  2,  2   )        :: hydro

  integer :: m, ii, jj, kk, i, j, k, i1, j1, k1
  integer :: i_t, i_qv, i_mu, i_ph, i_phb, i_mub, i_pb, i_qr, i_qs, i_qg
  real    :: dx, dxm, dy, dym, dz, dzm
  ...
  i_t  = 0; i_qv = 0; i_mu = 0; i_mub = 0; i_ph = 0; i_phb = 0; i_pb = 0;
  i_qr = 0; i_qs = 0; i_qg = 0;
  ...
  if( i_qr < 1 ) call get_variable3d(inputfile, 'QRAIN     ', ix, jx, kx, 1, qr )
  if( i_qs < 1 ) call get_variable3d(inputfile, 'QSNOW     ', ix, jx, kx, 1, qs )
  if( i_qg < 1 ) call get_variable3d(inputfile, 'QGRAUP    ', ix, jx, kx, 1, qg )
  ...
  if (obs_kk .le. 1) then
     hydro = qr(i1:i1+1,j1:j1+1,1:2) + qs(i1:i1+1,j1:j1+1,1:2) + qg(i1:i1+1,j1:j1+1,1:2)
     xb = dym*(dx*hydro(2,1,1) + dxm*hydro(1,1,1)) + dy*(dx*hydro(2,2,1) + dxm*hydro(1,2,1))
  else
     hydro = qr(i1:i1+1,j1:j1+1,k1:k1+1) + qs(i1:i1+1,j1:j1+1,k1:k1+1) + qg(i1:i1+1,j1:j1+1,k1:k1+1)
     xb = ( dzm * (dym*(dx*hydro(2,1,1) + dxm*hydro(1,1,1)) + dy*(dx*hydro(2,2,1) + dxm*hydro(1,2,1))) + &
            dz * (dym*(dx*hydro(2,1,2) + dxm*hydro(1,1,2)) + dy*(dx*hydro(2,2,2) + dxm*hydro(1,2,2))) )
  endif

  end subroutine xb_to_hydro
...
if ( i_t  == 0 ) call get_variable3d(inputfile, 'T_2       ', ix, jx, kx,   1, t)
if ( i_ph == 0 ) call get_variable3d(inputfile, 'PH_2      ', ix, jx, kx+1, 1, ph )
if ( i_phb== 0 ) call get_variable3d(inputfile, 'PHB       ', ix, jx, kx+1, 1, phb)
if ( i_qv == 0 ) call get_variable3d(inputfile, 'QVAPOR    ', ix, jx, kx,   1, qv )
